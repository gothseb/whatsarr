...
- Worker must call `WhatsAppAdapter`, update attempts/status, and never create duplicate jobs during retry.
- Preserve failure causes for logs without leaking secrets.
...
- Before implementation, read previous story context if present: `N:\windsurf\whatsarr\_bmad-output\implementation-artifacts\4-2-creer-l-outbox-de-notifications-whatsapp.md`.
