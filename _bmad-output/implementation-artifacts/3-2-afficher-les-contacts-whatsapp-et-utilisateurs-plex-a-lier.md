...
- Mapping UI should make non-notifiable users obvious without blocking other work.
- Keep display logic in web; mapping rules stay in API.
...
- Before implementation, read previous story context if present: `N:\windsurf\whatsarr\_bmad-output\implementation-artifacts\3-1-importer-les-utilisateurs-plex.md`.
...
- Added `/api/mapping` state with Plex users, WhatsApp contacts, linked contacts and non-notifiable counts.
- Added the Mapping UI tab with side-by-side Plex/contact visibility and mapping states.
