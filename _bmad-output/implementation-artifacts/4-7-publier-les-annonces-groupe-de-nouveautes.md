...
- Announcement jobs target the selected Groupe serveur only.
- Missing group must not create a job; log `WHATSAPP_GROUP_NOT_SELECTED`.
- The announcement publisher must consume routed availability events only: Overseerr for films/season 1, Plex for seasons 2+.
...
- Before implementation, read previous story context if present: `N:\windsurf\whatsarr\_bmad-output\implementation-artifacts\4-6-filtrer-les-nouveautes-recentes.md`.
