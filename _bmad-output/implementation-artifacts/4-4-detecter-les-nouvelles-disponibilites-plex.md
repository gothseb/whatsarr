...
- Route external signals into `media.availability.routed` before creating announcement jobs.
- Overseerr is the source of truth for films and season 1; Plex is the source of truth for seasons 2+ and individual episode additions.
- Duplicate detection must happen before creating announcement jobs and must dedupe season-level announcements by series/season.
...
- Before implementation, read previous story context if present: `N:\windsurf\whatsarr\_bmad-output\implementation-artifacts\4-3-executer-les-jobs-d-envoi-whatsapp.md`.
