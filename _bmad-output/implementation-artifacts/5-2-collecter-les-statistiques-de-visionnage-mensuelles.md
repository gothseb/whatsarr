...
As a Administrateur,
I want que Whatsarr collecte les vues des 30 derniers jours au moment du recap,
so that le recap mensuel reflete l'activite recente reelle du serveur.
...
**And** limite la periode aux 30 derniers jours avant l'execution planifiee.
...
- Use the chosen Tautulli/Plex source consistently and document source limitations.
- Do not send incomplete recaps automatically when stats collection fails.
...
- Before implementation, read previous story context if present: `N:\windsurf\whatsarr\_bmad-output\implementation-artifacts\5-1-configurer-les-bibliotheques-du-recap-mensuel.md`.
...
- Implemented Epic 5 monthly recap libraries, Tautulli rolling 30-day stats, distinct-user ranking, monthly recap outbox job creation, persistent recap status, operational logs and live dashboard/SSE status.
- Updated the recap period from the previous calendar month to the last 30 days at the configured execution time.
