...
- Recipient resolution must return every contact linked to the Plex user.
- This story enables later one-job-per-contact notification behavior.
...
- Before implementation, read previous story context if present: `N:\windsurf\whatsarr\_bmad-output\implementation-artifacts\3-3-associer-un-utilisateur-plex-a-un-contact-whatsapp.md`.
...
- Mapping model supports multiple WhatsApp contacts per Plex user and displays every linked contact.
- Recipient resolution returns every active linked contact and logs ignored inactive contacts.
